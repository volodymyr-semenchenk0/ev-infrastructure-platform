...
| `confidence_intervals` | JSON-документ | NOT NULL | Довірчі інтервали значень $C_i^*$ найкращих локацій ранжування, обчислені за перцентилями 2,5 % і 97,5 % накопиченої вибірки; структура – список об'єктів «`location_id`, середнє $\bar C_i^*$, нижня межа, верхня межа». Середнє слугує критерієм відбору топ-3 локацій, а несиметричні межі задають перцентилі |
