...
- Малюнки у тексті – плейсхолдер `![alt](images/...)` + підпис «Рис. X.Y.», SVG/PNG генерувати окремою сесією. До малюнку в тексті має бути посилання на нього, після малюнку має бути текст опис цього малюнка чи аналіз по ньому.
...
- При написанні або редагуванні будь-якого `.puml`-файлу обов'язково дотримуватись правил оформлення з `docs/ІНСТРУКЦІЇ_ДІАГРАМ.md` для відповідного типу діаграми (іменування дій, стереотипи, кратності, розмір шрифту тощо); конкретні правила не дублюються тут, щоб не протухали окремо від джерела
...
**Merge в main – лише через PR після чотирьох умов:**
1. Всі тести проходять (`make all` зелений)
2. Зміни верифіковані (uvicorn / vite піднімаються, API відповідає)
3. PR відкрито через `gh pr create`
4. Явний дозвіл користувача: «можна мерджити» або «merge»

```bash
# Відкрити PR
gh pr create --title "<тип>(<scope>): короткий опис" --body "..."

# Merge після дозволу користувача
gh pr merge --merge --delete-branch
git checkout main && git pull
```

Повний прогін перевірок (lint, typecheck, тести, build) запускати **один раз перед мерджем**, а не після кожного коміту. Під час розробки достатньо точкових перевірок зміненого; повний `make all` (і фронтенд-аналоги) – фінальний крок безпосередньо перед merge.

Ніколи не мерджити мовчки. Ніколи не комітити в main напряму — будь-яка зміна, включно з однорядковими правками, йде через окрему гілку і PR.
